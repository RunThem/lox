...
function _make_arguments(jsonfile, arguments, sourcefile)
  ...
  if not sourcefile then
    for _, arg in ipairs(arguments) do
      local sourcekind = try({
        function()
          return language.sourcekind_of(path.filename(arg))
        end,
      })
  ...
  "file": "%s"
    ...
    (_g.firstline and '' or ',\n'),
    _escape_path(os.projectdir()),
    table.concat(arguments_escape, '", "'),
    _escape_path(sourcefile)
  )
...
function _make_commands_for_objects(jsonfile, target, sourcebatch)
  ...
  if sourcekind and _sourcebatch_is_built(sourcebatch) then
    for index, sourcefile in ipairs(sourcebatch.sourcefiles) do
      ...
      local objectfile = projectdir .. '/' .. sourcebatch.objectfiles[index]
      local arguments =
        table.join(compiler.compargv(sourcefile, objectfile, { target = target, sourcekind = sourcekind }))
