...
function _sourcebatch_is_built(sourcebatch)
  ...
  if
    rulename == 'c.build'
    or rulename == 'c++.build'
    or rulename == 'asm.build'
    or rulename == 'cuda.build'
    or rulename == 'objc.build'
    or rulename == 'objc++.build'
  then
    return true
  end
...
function _add_target_binary(cmakelists, target, outputdir)
  ...
  cmakelists:print(
    'set_target_properties(%s PROPERTIES RUNTIME_OUTPUT_DIRECTORY "%s")',
    target:name(),
    _get_unix_path_relative_to_cmake(target:targetdir(), outputdir)
  )
end
...
function _add_target_static(cmakelists, target, outputdir)
  ...
  cmakelists:print('set_target_properties(%s PROPERTIES OUTPUT_NAME "%s")', target:name(), target:basename())
  cmakelists:print(
    'set_target_properties(%s PROPERTIES ARCHIVE_OUTPUT_DIRECTORY "%s")',
    target:name(),
    _get_unix_path_relative_to_cmake(target:targetdir(), outputdir)
  )
end
...
function _add_target_shared(cmakelists, target, outputdir)
  ...
  if target:is_plat('windows') then
    ...
    cmakelists:print(
      'set_target_properties(%s PROPERTIES LIBRARY_OUTPUT_DIRECTORY "%s")',
      target:name(),
      _get_unix_path_relative_to_cmake(target:targetdir(), outputdir)
    )
  end
...
function _add_target_sources(cmakelists, target, outputdir)
  ...
  if has_cuda then
    ...
    if devlink ~= nil then
      cmakelists:print(
        'set_target_properties(%s PROPERTIES CUDA_RESOLVE_DEVICE_SYMBOLS %s)',
        target:name(),
        devlink and 'ON' or 'OFF'
      )
    end
  end
end
  ...
  for _, filegroup in ipairs(filegroups) do
    ...
    if #sources > 0 then
      ...
      if mode and mode == 'plain' then
        cmakelists:print(
          'source_group(%s FILES ${%s_GROUP_SOURCE_LIST})',
          _get_unix_path(filegroup, outputdir),
          target:name()
        )
      else
        cmakelists:print(
          'source_group(TREE %s PREFIX %s FILES ${%s_GROUP_SOURCE_LIST})',
          rootdir,
          _get_unix_path(filegroup, outputdir),
          target:name()
        )
    ...
    if #recurse_sources > 0 then
      cmakelists:print(
        'FILE(GLOB_RECURSE %s_GROUP_RECURSE_SOURCE_LIST %s)',
        target:name(),
        table.concat(recurse_sources, ' ')
      )
      if mode and mode == 'plain' then
        cmakelists:print(
          'source_group(%s FILES ${%s_GROUP_RECURSE_SOURCE_LIST})',
          _get_unix_path(filegroup, outputdir),
          target:name()
        )
      else
        cmakelists:print(
          'source_group(TREE %s PREFIX %s FILES ${%s_GROUP_RECURSE_SOURCE_LIST})',
          rootdir,
          _get_unix_path(filegroup, outputdir),
          target:name()
        )
...
function _add_target_precompiled_header(cmakelists, target, outputdir)
  ...
  if precompiled_header then
    ...
    print(type(precompiled_header))
    cmakelists:print(
      '    $<$<COMPILE_LANGUAGE:%s>:${CMAKE_CURRENT_SOURCE_DIR}/%s>',
...
function _add_target_include_directories(cmakelists, target, outputdir)
  ...
  if configheader then
    cmakelists:print(
      ...
      target:name(),
      _get_unix_path(path.directory(configheader), outputdir)
    )
  end
end
...
function _add_target_languages(cmakelists, target)
  ...
  if languages then
    for _, lang in ipairs(languages) do
      ...
      if feature then
        cmakelists:print(
          'set_target_properties(%s PROPERTIES CXX_EXTENSIONS %s)',
          target:name(),
          has_ext and 'ON' or 'OFF'
        )
  ...
  if cmd.program then
    ...
    if opt and opt.curdir then
      command = '${CMAKE_COMMAND} -E chdir '
        .. _get_unix_path_relative_to_cmake(opt.curdir, outputdir)
        .. ' '
        .. command
    end
    return command
  elseif kind == 'cp' then
    if os.isdir(cmd.srcpath) then
      return string.format(
        '${CMAKE_COMMAND} -E copy_directory %s %s',
        _get_unix_path_relative_to_cmake(cmd.srcpath, outputdir),
        _get_unix_path_relative_to_cmake(cmd.dstpath, outputdir)
      )
    else
      return string.format(
        '${CMAKE_COMMAND} -E copy %s %s',
        _get_unix_path_relative_to_cmake(cmd.srcpath, outputdir),
        _get_unix_path_relative_to_cmake(cmd.dstpath, outputdir)
      )
    end
...
function _add_target_custom_commands_for_objectrules(cmakelists, target, sourcebatch, outputdir, suffix)
  ...
  local ruleinst =
    assert(target:rule(rulename) or project.rule(rulename) or rule.rule(rulename), 'unknown rule: %s', rulename)
